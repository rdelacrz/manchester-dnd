...
const address = process.env.PLAYWRIGHT_ADDRESS ?? '127.0.0.1:6793';
const baseURL = process.env.PLAYWRIGHT_BASE_URL ?? `http://${address}`;

export default defineConfig({
  testDir: path.join(repositoryRoot, 'tests/browser'),
  testMatch: 'slice3-narration.spec.ts',
  outputDir: path.join(repositoryRoot, 'target/playwright/slice3-test-results'),
  workers: 1,
  retries: 0,
  reporter: [
    ['list'],
    [
      'html',
      {
        outputFolder: path.join(repositoryRoot, 'target/playwright/slice3-report'),
        open: 'never',
      },
    ],
  ],
  ...
  webServer: {
    command: path.join(
      repositoryRoot,
      'tests/browser/support/run-slice3-browser-server.sh',
    ),
    cwd: repositoryRoot,
    ...
    env: {
      ...
      RNG_MASTER_KEY_FILE: '.runtime-private/playwright/slice3/rng-master.key',
